...
```{r mcmc_neff-compare, fit.height=10}
...
```{r mcmc_acf, fig.width=10, fig.height=10}
compare_cp_ncp(
...
plot (in red) of the divergences to a trace plot of parameter draws.
Typically we can see that at least one of the chains is getting stuck wherever
there is a cluster of many red marks:
...
```{r mcmc_nuts_divergence, fig.height=10}
...
indicate that some part of the posterior isn't being explored and
`lp|Divergence` will have lighter tails than `lp|No divergence`.
...
```{r fit-adapt-delta, results='hide', message=FALSE, fig.height=10}
...
```{r mcmc_nuts_divergence-3, fig.height=10}
...
```{r mcmc_nuts_energy-3, message=FALSE, fig.height=10}
compare_cp_ncp(
...
```{r mcmc_nuts_energy-4, message=FALSE, fig.height=10}
