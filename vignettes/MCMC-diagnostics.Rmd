...
[*Plotting MCMC draws using the bayesplot package*]("MCMC.html"), and graphical posterior
predictive checks are covered in [*Graphical posterior predictive checks using
the bayesplot package*]("PPC.rmd").
...
Markov chain(s) have converged. The __bayesplot__ package provides various plotting functions for visualizing 
Markov chain Monte Carlo (MCMC) diagnostics after fitting a Bayesian model. MCMC
draws from any package can be used with __bayesplot__, although there are a few
diagnostic plots that are specifically intended to be used for [Stan](http://mc-stan.org/) models. 

To demonstrate, in this vignette we'll use the eight schools example discussed in  Rubin (1981), Gelman et al (2013), and the [RStan Getting Started](https://github.com/stan-dev/rstan/wiki/RStan-Getting-Started#how-to-use-rstan) wiki. This is a simple hierarchical meta-analysis model with data consisting of point estimates `y` and standard errors `sigma` from analyses of test prep 
...
$$
\begin{align*}
y_j &\sim N(\theta_j, \sigma^2_j), \quad j = 1,\dots,J \\
\theta_j &\sim N(\mu, \tau^2), \quad j = 1, \dots, J \\
\mu &\sim N(0, 10^2) \\
\tau &\sim Cauchy^+(0, 10)
\end{align*}
$$

which in Stan code can be written:
...
model {
  mu ~ normal(0, 10);  // Stan uses SD for normal
  tau ~ cauchy(0, 10);
  theta ~ normal(mu, tau);
...
We'll also fit the same statistical model but using the so-called non-centered parameterization, which replaces the vector $\theta$ with a vector $\eta$ of a priori i.i.d. standard normal parameters and then contructs $\theta$ deterministically from $\eta$, $\mu$, $\tau$:
$$
\begin{align*}
\theta_j &= \mu + \tau \eta_j \\
\eta_j &\sim N(0,1).
...
```
Again, these are the same _statistical_ model, however they have very different practical implications for MCMC. Using the __bayesplot__ diagnostic plots, we'll see that in this case we need the non-centered parameterization to make the posterior geometry amenable to sampling.
...
Fit both models and extract posterior draws:
...
For now, ignore any warnings about divergent transitions after warmup. We will come back to those later in the vignette in the
[Diagnostics for the No-U-Turn Sampler](#diagnostics-for-the no-u-turn-sampler) section.
...
We can see the names of the parameters with the concerning $\hat{R}$ values by turning
...
Because the draws within a Markov chain are _not_ independent if there is
...
Even for models fit using **rstan** the parameterization can make
a big difference. Here are the $n_{eff}/N$ plots for `fit1` and `fit2` side by side.

```{r mcmc_neff-compare}
gridExtra::grid.arrange(
  mcmc_neff(neff_ratio(fit1, pars = c("theta", "mu", "tau"))),
  mcmc_neff(neff_ratio(fit2, pars = c("theta", "mu", "tau")))
)
```

Because of the difference in parameterization, the effective sample sizes are much better for the second model.
...
`mcmc_acf` (line plot) or `mcmc_acf_bar` (bar plot) functions.
For the selected parameters, these functions show the autocorrelation for each Markov chain separately up to a user-specified number of lages.

Here we can again see a difference when comparing the two parameterizations
of the same model. For model 1, $\theta_1$ is the primitive parameter for school 1, whereas for the non-centered parameterization in model 2 the primitive parameter is $\eta_1$ (and $\theta_1$ is later constructed from $\eta_1$, $\mu$, and $\tau$):
...
and NUTS in particular. See Betancourt and Girolami (2013), Betancourt (2016), and Stan Development Team (2016) for
...
To look at some information about the divergences we can use the `mcmc_nuts_divergence` function:

```{r, mcmc_nuts_divergence}
mcmc_nuts_divergence(np1, lp1)
```

In the top panel we see the distribution of the log-posterior
when there was no divergence vs the distribution when there was a divergence. Divergences often indicate that some part of the posterior isn't being explored and we will see that `lp|Divergence` has lighter tails than `lp|No divergence`.

The bottom panel shows the same thing but instead of the log-posterior the NUTS
acceptance statistic is shown.

Specifying the optional `chain` argument will overlay the plot
just for a particular Markov chain on the plot for all chains
combined:
...
but we have far fewer of them to worry about. If there are only a few divergences we can often get rid of them by increasing the target acceptance rate (`adapt_delta`), which has the effect of lowering the stepsize used by the sampler and allowing for more precisely exploration of the target distribution. On the other hand, for the first model, increasing `adapt_delta` will not entirely solve the problem. 
...
distribution $\pi_{\Delta E}$, which should look the same under ideal conditions
(see Betancourt, 2016, for details). For example, compare the energy diagnostic
plots for the models we fit above. The better parameterization for this model
(non-centered) is in the bottom panel:

```{r, energy-plot-1, message=FALSE}
gridExtra::grid.arrange(
...
If we force the stepsize to a smaller value to allow for better exploration
of the target distribution the difference between the parameterizations is
even more apparent:

```{r, energy-plot-2, message=FALSE}
...
See Betancourt (2016) for more on this particular example as well as the general theory behind the energy plots.
