...
observation. For a well-calibrated model, these transformations result in 
...
With the introduc tion of the `method = "correlated"` option, the three 
...
gamma <- quantile(
  posterior::uniformity_test(pit = pit, test = "POT")$pointwise, 0.85
)[1]
p2 <- ppc_loo_pit_ecdf(
  pit = pit, method = "correlated",
  plot_diff = TRUE,
  ...
  labs(subtitle = sprintf("gamma = %.0f", gamma))
