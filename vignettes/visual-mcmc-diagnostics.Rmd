...
fit_ncp <- sampling(schools_mod_ncp, data = schools_dat, seed = 457721433)
```
...
[Divergent transitions after warmup](http://mc-stan.org/misc/warnings.html#divergent-transitions-after-warmup).
...
Those functions require more information than simply the posterior samples, in
particular the log of the posterior density for each sample and some
...
(although it may be larger in some cases). The larger the ratio of $n_{eff}$ to
$N$ the better.
...
_Journal of the Royal Statistical Society Series A_, accepted for publication. 
