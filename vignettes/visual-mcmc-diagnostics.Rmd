...
```{r, compile-models, eval=FALSE, cache=TRUE}
...
fit_cp <- sampling(schools_mod_cp, data = schools_dat, seed = 80321405423)
fit_ncp <- sampling(schools_mod_ncp, data = schools_dat, seed = 45772143245)
```
...
In addition to these functions that are only for HMC/NUTS diagnostic plots, 
in the **Divergent transitions** section below, we will also demonstrate 
that some of the functions from the 
[_Plotting MCMC draws_](http://mc-stan.org/bayesplot/articles/plotting-mcmc-draws.html)
...
variables (which manifests as narrow bivariate plots) as well the presence of
...
100th iteration and zoom in to investigate, using the `window` parameter:

```{r, mcmc_trace_zoom}
mcmc_trace(posterior_cp, pars = "tau", np = np_cp, window = c(50,150)) + 
  xlab("Post-warmup iteration")
```
...
fit_cp_2 <- sampling(schools_mod_cp, data = schools_dat,
                     control = list(adapt_delta = 0.99), seed = 978245244321)
...
fit_cp_100iter <- sampling(schools_mod_cp, data = schools_dat, 
                           chains = 2, iter = 100, seed = 4512789655)
```
