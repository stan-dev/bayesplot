...
us diagnose divergences. We may however zoom in to investigate, using the `window` parameter:

```{r echo=FALSE}
#A check that the chosen window still relevant
n_divergent_in_window <- np_cp %>% filter(Parameter == "divergent__" & Value == 1 & Iteration >= 80 & Iteration <= 230) %>% nrow()
...
```{r, mcmc_trace_zoom}
mcmc_trace(posterior_cp, pars = "tau", np = np_cp, window = c(80,230)) + 
  xlab("Post-warmup iteration")
```
