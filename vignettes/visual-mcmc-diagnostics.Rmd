...
[ShinyStan](http://mc-stan.org/shinystan/) package.
...
fit_cp <- sampling(schools_mod_cp, data = schools_dat, seed = 803214054)
fit_ncp <- sampling(schools_mod_ncp, data = schools_dat, seed = 457721432)
```
...
We'll
have a look at diagnosing the source of the divergences first and then dive into
some diagnostics that should be checked even if there are no warnigns from the 
sampler.
...
Note that some of the functions from the 
[_Plotting MCMC draws_](http://mc-stan.org/bayesplot/articles/plotting-mcmc-draws.html)
vignette also take optional arguments that can be used to display important
HMC/NUTS diagnostic information, as we'll se below.


### Divergent transitions

When running the Stan models above,
there were warnings about divergent transitions.
Here we'll look at diagnosing the source of divergences through visualizations.
...
deviation, goes to zero and the values of the `theta`s are essentially fixed.
This makes `tau` immediately suspect. 
See [Gabry et al. (2018)](https://arxiv.org/abs/1709.01449) for another example
of the parallel coordinates plot.


#### mcmc_pairs

The `mcmc_pairs` function can be also be used to look at multiple
...
lower bound of zero. That is, even though the samples for `tau` returned from
...
blob/cloud and the divergences go away. Gabry et al. (2018) has further
...
comparison with scatterplot of `theta[1]` vs `log(tau)` from the centered
parameterization that we made above. We will also force the plots to have the
same $y$-axis limits, which will make the more important difference much more
apparent:
...
parameterization allowed us to obtain samples from the funnel distribution
...
and we see a clustering of divergences and no samples in the tail of the
distribution.
...
us diagnose divergences. We may however notice the cluster of divergences around
950th iteration and zoom in to investigate, using the `window` parameter:

```{r echo=FALSE}
#A check that the chosen window still relevant
n_divergent_in_window <- np_cp %>% filter(Parameter == "divergent__" & Value == 1 & Iteration > 900 & Iteration < 1000) %>% nrow()

if(n_divergent_in_window < 10) {
  cat("Divergences happened at: ")
   np_cp %>% filter(Parameter == "divergent__" & Value == 1) %>% select(Iteration) %>% get("Iteration", .) %>% print()
  stop("Too few divergences in the selected window for traceplot zoom. Change the window or the random seed.")
}
```


```{r, mcmc_trace_zoom}
mcmc_trace(posterior_cp, pars = "tau", np = np_cp, window = c(900,1000)) + 
  xlab("Post-warmup iteration")
```

What we see here is that chains often get stuck as `tau` approaches zero and
spend substantial time in the same region of parameter space. This is just
...
fit_cp_2 <- sampling(schools_mod_cp, data = schools_dat,
                     control = list(adapt_delta = 0.99), seed = 978245244)
fit_ncp_2 <- sampling(schools_mod_ncp, data = schools_dat,
                      control = list(adapt_delta = 0.99), seed = 843256842)
```
...
fit_cp_100iter <- sampling(schools_mod_cp, data = schools_dat, 
                           chains = 2, iter = 100, seed = 671254821)
```
