...
[_Graphical posterior predictive checks_](http://mc-stan.org/bayesplot/articles/graphical-ppcs.html)
vignette.
...
are specifically intended to be used for [Stan](http://mc-stan.org/) models (or
models fit using the same algorithms as Stan).

**Documentation:**

* `help("MCMC-diagnostics")` 
* [mc-stan.org/bayesplot/reference/MCMC-diagnostics](http://mc-stan.org/bayesplot/reference/MCMC-diagnostics.html)
...
methods defined for models fit using the **rstan**, **rstanarm** and **brms** packages.
...
(although it may be larger in some cases). The 
...
user-specified number of lags. Autocorrelation is bad (it means samples are 
not independent) and you want it to drop quickly to zero with increasing lag.
...
* `help("MCMC-nuts")` 
* [mc-stan.org/bayesplot/reference/MCMC-nuts](http://mc-stan.org/bayesplot/reference/MCMC-nuts.html)
...
Those functions require more information than simply the posterior samples,
in particular the log of the posterior density for each sample and some NUTS-specific
diagnostic values may be needed. 
The **bayesplot** package provides generic functions `log_posterior` and 
`nuts_params` for extracting this information from fitted 
model objects. Currently methods are provided for models fit using the **rstan**,
 **rstanarm** and **brms** packages, although it is not difficult to define additional 
...
*Note:* some of the functions demonstrated in the 
[_Plotting MCMC draws_](http://mc-stan.org/bayesplot/articles/plotting-mcmc-draws.html)
...
Here we'll look at diagnosing the source of divergences through visualisations.

#### mcmc_parcoord

The parallel coordinates plot (`mcmc_parcoord`) is probably the first plot to have look
at if you have no idea, where the divergences in your model might be coming from. This function 
works in general without including information about the divergences, but if the optional `np` 
argument is used to pass NUTS parameter information, 
...
The `mcmc_parcoord` plot shows one line per iteration, connecting the parameter values
at this iteration. This lets you see any global patterns in the divergences. Here,
you may notice that divergences in the centered parametrization happen 
exclusively when  `tau`, the hierarchical standard deviation, goes to zero and
the values of the `theta`s are essentially fixed. This makes `tau` immediately 
suspect.
See Gabry et al. (2018) for more another example of the parallel
coordinates plot.


#### mcmc_pairs

The `mcmc_pairs` function can be used also be used to look at multiple parameters at once,
but unlike `mcmc_parcoord` that works well even with several dozen parameters, `mcmc_pairs`
is more useful for up to ~8 params. It shows univariate histograms and bivariate scatter plots
for selected parameters and is especially useful in identifying collinearity between variables
(which demonstrate as narrow bivariate plots). Let's look how `tau` interacts with other
variables, using only one of the `theta`s for clarity:

```{r, mcmc_pairs}
mcmc_pairs(posterior_cp, np = np_cp, pars = c("mu","tau","theta[1]"))
```

Note that each bivariete plot is present twice - by default each of those contain half of the chains,
so you also get to see if the chains produced similar results. Here, the interaction of `tau` and 
`theta[1]` seems most interesting, as it concetrates the divergences into a tight region.

Before we move on, note that further examples are also provided via `help("mcmc_pairs")`.


#### mcmc_scatter

Using the `mcmc_scatter` function (with optional argument `np`) we can 
look at a single bivariate plot to investigate it more closely. So let's look
at `tau` and `theta[1]` parameters. We will also use the `transform` argument to look at log of
`tau`.
...
This picture shows a funnel-like shape of the distribution. This one in particular is known as 
Neal's funnel and we clearly see that NUTS has problems exploring the narrow tip of the funnel. 
The main problem is that large steps are required to explore the wide arrea efficiently, but those
steps become too large in the narrow region. The required step size is connected to the value of `tau` -
large `tau` allows large variation in `theta` (and requires large steps) while small `tau` requires small
steps in `theta`. The non-centered parametrization avoids this by sampling the
`eta` parameter which is independent of `tau` and computing `theta` from `eta`,`mu` and `tau` afterwards.
Let's look at the same plot, but with the `eta` parameter from non-centered parametrization instead of `theta`:
...
We can see that the funnel-like shape is replaced by a somewhat Gaussian blob and the divergences go away. Gabry et al. (2018) has some further discussion of this example.


#### mcmc_trace

Annother useful diagnostic plot is the trace plot showing the evolution of parameter samples
over iterations for each chain.
...
cluster of many red marks. So let's investigate what it tells us about `tau` in the centered 
parametrization:
...
First thing to note is that all chains seem to be exploring the same region of parameter 
values, which is a good sign. But the plot is too crowded to help us 
diagnose divergences. We may however notice the cluster of divergences after 600th iteration
and zoom in to investigate, using the `window` parameter:

```{r, mcmc_trace_zoom}
mcmc_trace(posterior_cp, pars = "tau", np = np_cp, window = c(600,700)) + 
  xlab("Post-warmup iteration")
```

What we see here is that chains often get stuck as `tau` approaches zero and spend substantial
time in the same region of parameter space. This is just another indication that there is 
problematic geometry at $\tau \simeq 0$ - healthy chains jump up and down frequently.

#### mcmc_nuts_divergence

To understand how the divergences interact with the model globally, we can use the 
...
http://mc-stan.org/rstan, https://CRAN.R-project.org/package=rstan
