#' PPC vs predictors
#'
#' Compare posterior (or prior) predictive draws `yrep` to observations `y`,
#' plotted against a predictor (or time) variable `x`.
#' See the **Plot Descriptions** section, below.
#'
#' @name PPC-vs-predictors
#' @family PPCs
#'
...
ppc_curve_overlay <-
  function(y,
    ...
    data <- ppc_curve_data(y = y, yrep = yrep, x = x, group = dots$group)
    ggplot(
      data = dplyr::filter(data, !.data$is_y),
      mapping = aes_(x = ~ x, y = ~ value)
    ) +
      geom_line(
        mapping = aes_(group = ~ rep_label, color = "yrep"),
        size = size,
        alpha = alpha
      ) +
     geom_point(
      ...
      scale_color_ppc() +
      labs(y = NULL, x = expression(italic(x))) +
      bayesplot_theme_get()
  }

#' @rdname PPC-vs-predictors
#' @export
ppc_curve_overlay_grouped <-
  function(y,
           yrep,
           x = NULL,
           group,
    ...
    g <- eval(ungroup_call("ppc_curve_overlay", call), parent.frame())
