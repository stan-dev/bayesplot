...
#' functions in \pkg{ppcheck} will be encouraged to include \code{pp_check} methods
#' in their package, preserving the same naming conventions for posterior
...
#' @return The exact form of the value returned by \code{pp_check} may vary by the
...
#'   can include a method \code{pp_check.foo} that prepares the appropriate inputs
#'   (\code{y}, \code{yrep}, etc.) for the \pkg{ppcheck} functions. The
#'   \code{pp_check.foo} method may, for example, let the user choose between various
#'   \pkg{ppcheck} plots, calling the functions from \pkg{ppcheck} internally as
