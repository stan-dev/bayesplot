...
// Piecewise kernel value estimation (Chen 1999)
 double bc_kde_calc(double xs, double b,  Map<VectorXd> pvals){
  ...
  if ((xs >= 2*b) && (xs <= (1 - 2*b))){
    tmp = vec_dbeta(pvals, xs/b, (1 - xs)/b); // helper custom fuction
    d = tmp.sum() / tmp.size();               // vector mean
  } else if ((xs >= 0) && (xs < 2*b)) {
    tmp = vec_dbeta(pvals, rho(xs, b), (1 - xs)/b);
    d = tmp.sum() / tmp.size();
  } else if ((xs > (1 - 2*b)) && (xs <= 1)) {
    tmp = vec_dbeta(pvals, xs/b, rho(1 - xs, b));
    d = tmp.sum() / tmp.size();
  } else {
    d = 0.0;
  }
...
// [[Rcpp::export]]
RowVectorXd bc_dunif(Map<VectorXd> xs, Map<VectorXd> pvals, 
