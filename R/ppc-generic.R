...
#' methods accompanying the \code{ppc} generic. The intent is to provide a
#' generic so authors of other \R packages who wish to provide interfaces to the
#' functions in \pkg{ppcheck} will be encouraged to include \code{ppc} methods
...
#'   can include a method \code{ppc.foo} that prepares the appropriate inputs
#'   (\code{y}, \code{yrep}, etc.) for the \pkg{ppcheck} functions. The
#'   \code{ppc.foo} method may, for example, let the user choose between various
