...
Many of the
[PPC](https://mc-stan.org/bayesplot/reference/PPC-overview.md) functions
in **bayesplot** can be used with discrete data. The small subset of
these functions that can *only* be used if `y` and `yrep` are discrete
are documented on this page. Currently these include rootograms for
count outcomes and bar plots for ordinal, categorical, and multinomial
outcomes. See the **Plot Descriptions** section below.
