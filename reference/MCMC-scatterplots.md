...
- pars:
  ...
  selection](https://mc-stan.org/bayesplot/reference/tidy-params.md)
  page.
...
- chains, draws, nuts:
  ...
  - For models fit using NUTS, the `nuts` argument to
    ...
    [`NUTS()`](https://mc-stan.org/bayesplot/reference/MCMC-nuts.md)
...
- `mcmc_scatter()`:
  ...
  the plot (in this case the approximate location of divergences). For
  more on why the scatter plot with divergences is a useful diagnostic
  tool see [Gabry et al.
...
- `mcmc_pairs()`:
  ...
  [NUTS](https://mc-stan.org/bayesplot/reference/MCMC-nuts.md) can be
  added to the pairs plot using the `lp`, `np`, and `np_style`
