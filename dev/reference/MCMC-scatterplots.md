...
#> Error in geom_point(aes(color = divs_j_fac, size = divs_j_fac), shape = np_style$shape[["div"]],     alpha = np_style$alpha[["div"]], na.rm = TRUE): Problem while computing aesthetics.
#> ℹ Error occurred in the 2nd layer.
#> Caused by error in `check_aesthetics()`:
#> ! Aesthetics must be either length 1 or the same as the data (1001).
