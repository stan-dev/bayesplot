# Authors and Citation

## Authors

- **Jonah Gabry**. Author, maintainer.

- **Tristan Mahr**. Author. [](https://orcid.org/0000-0002-8890-5116)

- **Paul-Christian Bürkner**. Contributor.

- **Martin Modrák**. Contributor.

- **Malcolm Barrett**. Contributor.

- **Frank Weber**. Contributor.

- **Eduardo Coronado Sroka**. Contributor.

- **Teemu Sailynoja**. Contributor.

- **Aki Vehtari**. Contributor.

- **Behram Ulukır**. Contributor.

## Citation

Source:
[`inst/CITATION`](https://github.com/stan-dev/bayesplot/blob/fix-pkgdown-action/inst/CITATION)

Gabry J, Mahr T (2025). “bayesplot: Plotting for Bayesian Models.” R
package version 1.14.0.9000, <https://mc-stan.org/bayesplot/>.

    @Misc{,
      title = {bayesplot: Plotting for Bayesian Models},
      author = {Jonah Gabry and Tristan Mahr},
      year = {2025},
      note = {R package version 1.14.0.9000},
      url = {https://mc-stan.org/bayesplot/},
    }

Gabry J, Simpson D, Vehtari A, Betancourt M, Gelman A (2019).
“Visualization in Bayesian workflow.” *J. R. Stat. Soc. A*, **182**,
389-402. [doi:10.1111/rssa.12378](https://doi.org/10.1111/rssa.12378).

    @Article{,
      title = {Visualization in Bayesian workflow},
      author = {Jonah Gabry and Daniel Simpson and Aki Vehtari and Michael Betancourt and Andrew Gelman},
      year = {2019},
      journal = {J. R. Stat. Soc. A},
      volume = {182},
      issue = {2},
      pages = {389-402},
      doi = {10.1111/rssa.12378},
    }
