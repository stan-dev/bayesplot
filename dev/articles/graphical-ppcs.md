...
This vignette focuses on graphical posterior predictive checks (PPC).
...
and MCMC diagnostics are covered in the [*Visual MCMC
...
predictive distribution ([Gabry et al, 2019](#gabry2019)).
...
by a model after using the observed data \\y\\ (a vector of \\N\\
outcome values) to update our beliefs about unknown model parameters
\\\theta\\. The posterior predictive distribution for observation
\\\widetilde{y}\\ can be written as \\p(\widetilde{y} \\\|\\ y) = \int
p(\widetilde{y} \\\|\\ \theta) \\ p(\theta \\\|\\ y) \\ d\theta.\\
Typically we will also condition on \\X\\ (a matrix of predictor
variables).

For each draw (simulation) \\s = 1, \ldots, S\\ of the parameters from
the posterior distribution, \\\theta^{(s)} \sim p(\theta \\\|\\ y)\\, we
draw an entire vector of \\N\\ outcomes \\\widetilde{y}^{(s)}\\ from the
posterior predictive distribution by simulating from the data model
conditional on parameters \\\theta^{(s)}\\. The result is an \\S \times
N\\ matrix of draws \\\widetilde{y}\\.
...
they can be thought of as replications of the outcome \\y\\ rather than
predictions for future observations (\\\widetilde{y}\\ using predictors
\\\widetilde{X}\\). This corresponds to the notation from Gelman et
al. (2013) and is the notation used throughout the package
documentation.
...
replications. The names of the **bayesplot** plotting functions for
...
**bayesplot** package we’ll use an example of comparing Poisson and
Negative binomial regression models from one of the **rstanarm**
[package vignettes](https://mc-stan.org/rstanarm/articles/count.html)
...
> apartments. \[…\] The regression predictors for the model are the
> pre-treatment number of roaches `roach1`, the treatment indicator
> `treatment`, and a variable `senior` indicating whether the apartment
> is in a building restricted to elderly residents. Because the number
> of days for which the roach traps were used is not the same for all
> apartments in the sample, we include it as an exposure \[…\].
...
**rstan** package we could create `yrep` in the `generated quantities`
block of the Stan program or by doing simulations in R after fitting the
...
![](graphical-ppcs_files/figure-html/ppc_dens_overlay-1.png)
...
![](graphical-ppcs_files/figure-html/ppc_dens_overlay-2-1.png)

See Figure 6 in [Gabry et al. (2019)](#gabry2019) for another example of
using `ppc_dens_overlay`.
...
![](graphical-ppcs_files/figure-html/ppc_hist-1.png)
...
![](graphical-ppcs_files/figure-html/ppc_hist-nb-1.png)
...
![](graphical-ppcs_files/figure-html/ppc_hist-nb-2-1.png)
...
![](graphical-ppcs_files/figure-html/ppc_stat-1.png)
...
![](graphical-ppcs_files/figure-html/ppc_stat-nb-1.png)
...
![](graphical-ppcs_files/figure-html/ppc_stat-max-3.png)

See Figure 7 in [Gabry et al. (2019)](#gabry2019) for another example of
using `ppc_stat`.
...
intervals, distributions of predictive errors, and more. For links to
the documentation for all of the various PPC plots see
[`help("PPC-overview")`](https://mc-stan.org/bayesplot/dev/reference/PPC-overview.md)
from R or the [online
documentation](https://mc-stan.org/bayesplot/reference/index.html#section-ppc)
on the Stan website.
...
ending in `_grouped` and will accept an additional argument `group`. The
...
![](graphical-ppcs_files/figure-html/ppc_stat_grouped-1.png)
...
method needs to do is provide the `y` vector and `yrep` matrix arguments
to the various plotting functions included in **bayesplot**.

### Defining a `pp_check` method

Here is an example for how to define a simple `pp_check` method in a
package that creates fitted model objects of class `"foo"`. We will
define a method `pp_check.foo` that extracts the data `y` and the draws
from the posterior predictive distribution `yrep` from an object of
class `"foo"` and then calls one of the plotting functions from
**bayesplot**.
...
![](graphical-ppcs_files/figure-html/pp_check-2-1.png)
...
the `pp_check` methods in the
[**rstanarm**](https://CRAN.R-project.org/package=rstanarm) and
[**brms**](https://CRAN.R-project.org/package=brms) packages.

  

## References

Buerkner, P. (2017). brms: Bayesian Regression Models using Stan. R
package version 1.7.0. <https://CRAN.R-project.org/package=brms>
