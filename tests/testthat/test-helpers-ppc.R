...
# Test for influential_points_idx --------------------------------------------

test_that("influential_points_idx handles empty and no positive values", {
  # Empty vector
  result <- influential_points_idx(numeric(0), 0.05)
  expect_equal(result, integer(0))
  expect_type(result, "integer")
  
  # Mixed with no positive values
  result <- influential_points_idx(c(-1, 0, -2), 0.05)
  expect_equal(result, integer(0))
})

test_that("influential_points_idx handles simple cases", {
  # x = c(1, 2, 3), alpha = 0.05
  # target = qcauchy(0.95) ~= 6.314
  # sum(x) = 6
  # we need cumsum >= 6 - 6.314 = -0.314
  # So we need at least 1 element
  x <- c(1, 2, 3)
  result <- influential_points_idx(x, 0.05)
  expect_type(result, "integer")
  expect_true(length(result) >= 1)
  expect_true(all(result %in% seq_along(x)))
})

test_that("influential_points_idx returns indices in correct order", {
  # Values should be sorted by decreasing value
  # x = c(1, 5, 2, 4, 3) -> positive indices: 1, 2, 3, 4, 5
  # Sorted by value: 5, 4, 3, 2, 1 -> indices: 2, 4, 5, 3, 1
  x <- c(1, 5, 2, 4, 3)
  result <- influential_points_idx(x, 0.5)
  if (length(result) > 1) {
    values <- x[result]
    expect_equal(values, sort(values, decreasing = TRUE))
  }
})