...
testthat::test_that("ensure different modalities match", {
  SEED <- 6

  data("roaches", package = "rstanarm")
  roaches$sqrt_roach1 <- sqrt(roaches$roach1)

  fit_nb <- brms::brm(y ~ sqrt_roach1 + treatment + senior + offset(log(exposure2)),
             data = roaches,
             family = brms::negbinomial,
             prior = brms::prior(normal(0, 1), class = b),
             refresh = 0)

  fit_nb <- brms::add_criterion(fit_nb, criterion = "loo")
  fit_nb <- brms::add_criterion(
    fit_nb, criterion = "loo", moment_match = TRUE, save_psis = TRUE,
    overwrite = TRUE
  )
  
  #1
  set.seed(SEED)
  y_rep <- posterior_predict(fit_nb)

  p1 <- ppc_loo_pit_ecdf(fit_nb$data$y, y_rep, 
    psis_object = loo(fit_nb)$psis_object, method = "correlated")

  #2
  p2 <- ppc_pit_ecdf(
    pit = pareto_pit(y_rep, fit_nb$data$y, 
      weights.importance_sampling(loo(fit_nb)$psis_object, log=TRUE), log=TRUE),
    method = "correlated")

  #3
  p3 <- ppc_pit_ecdf(
    pit = rstantools::loo_pit(y_rep, fit_nb$data$y, 
      weights.importance_sampling(loo(fit_nb)$psis_object, log=TRUE), log=TRUE),
    method = "correlated")
